...
syntax match httpfileKeywords "\v^%(GET|POST|HEAD|PUT|DELETE|RSOCKET|RPC|FNF|STREAM|METADATA_PUSH|GRPC|GRAPHQL|GRAPHQLWS|GRAPHQLWSS|DUBBO|SOFA|TARPC|MSGPACK|JSONRPC|THRIFT|ZEROREQ|MAIL|ALIYUN|MEMCACHE|AWS|AWSPOST|AWSDELETE|AWSPUT|PUB|SUB|RSET|HMSET|EVAL|SSH)\s"
syntax match httpfileHeaders "\v^%(Accept|Accept-Charset|Accept-Encodingg|Accept-Datetime|Accept-Language|Authorization|Content-Type|Cache-Control|Cookie|Content-Length|Content-MD5|Date|From|Host|If-Modified-Since|Max-Forwards|Origin|Pragma|Proxy-Authorization|Range|Referer|User-Agent|X-Requested-With|X-Forwarded-For|X-Forwarded-Host|X-Forwarded-Proto|X-Csrf-Token|X-JSON-Type|Subject|Reply-to|X-JSON-Schema|X-JSON-Type|X-JSON-Path|X-Region-Id|X-GraphQL-Variables|X-SSH-Private-Key|X-Args-0|X-Args-1|X-Args-2|X-Args-3|X-Args-4|X-Args-5):"
